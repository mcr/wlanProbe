...
def connectedSchedule(
    ...
    # send from new queue
    print("XXX calling mqttSendQueue")
    printDebug("sendqueue", ioMqttCounter)
...
def startupSequenz(inNic):
    ...
    for scanTime in scanTimeList:
        for scanData in scanDataList:
            GLOBAL_MESSAGE_QUEUE.addMsg({"seconds": scanTime,
                                 "messageType" : MESSAGE_TYPES["wlanScan"],
                                 "messageContent": scanData})
    ...
    for failedConnectTime in failedConnectTimeList:
        GLOBAL_MESSAGE_QUEUE.addMsg({"seconds": failedConnectTime,
                                     "messageType" : MESSAGE_TYPES["connect"],
                                     "messageContent": connectDataFailed})
    ...
    GLOBAL_MESSAGE_QUEUE.addMsg({"seconds": outConnectTime,
                                 "messageType" : MESSAGE_TYPES["connect"],
                                 "messageContent": connectDataSuccess})
